...
<section class="content-header">
    <h1>
        ...
        <div class="col-md-2">
            <a href="{{ url('admin/addcoin') }}" class="btn btn-block btn-primary add-btn-primary pull-right">{{trans('adminlabels.add')}}</a>
        </div>       
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-body">
                    <table id="CoinListingTable" class="table table-striped">
                        <thead>
                            <tr>
							    <th>{{trans('adminlabels.coins')}}</th>
                                <th>{{trans('adminlabels.coinprice')}}</th>
								<th>{{trans('adminlabels.coindiscount')}}</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var table = $('#CoinListingTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!! route('.getcoin') !!}',
            columns: [
                {data: 'c_coins', name: 'c_coins'},
                {data: 'c_price', name: 'c_price'},
                 {data: 'c_discount', name: 'c_discount'},
