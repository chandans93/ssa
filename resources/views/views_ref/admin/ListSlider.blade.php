...
<section class="content-header">
    <h1>
        <div class="col-md-10">
            {{trans('adminlabels.slidertemplete')}}
        </div>
        <div class="col-md-2">
            <a href="{{ url('admin/addslider') }}" class="btn btn-block btn-primary add-btn-primary pull-right">{{trans('adminlabels.add')}}</a>
        </div>       
            ...
            <div class="box box-primary">
                <div class="box-body">
                    <table id="TempleteListingTable" class="table table-striped">
                        <thead>
                            <tr class="filters">
                                <th>{{trans('adminlabels.sliderimage')}}</th>
                                <th>{{trans('adminlabels.redirectionlink')}}</th>
                                <th>{{trans('adminlabels.displayStatus')}}</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var table = $('#TempleteListingTable').DataTable({
            ...
            columns: [
                {data: 'hps_image', name: 'hps_image'},
                {data: 'hps_redirection_link', name: 'hps_redirection_link'},
                {data: 'hps_display_status', name: 'hps_display_status'},
